                ...
                enum list_loc_t where)   /* insert location */
{
  ...
  else if (where == Lip_4 || where == Lip_8)
  {
    int i = (where == Lip_4) ? 0 : 15;

    /* Count associativity dynamically */
    int assoc = 0;
    struct cache_blk_t *tmp = set->way_head;
    while (tmp)
    {
      assoc++;
      tmp = tmp->way_next;
    ...
    /* Clamp i to [0, assoc-1] */
    if (i < 0)
      i = 0;
    if (i >= assoc)
      i = assoc - 1;

    /* Start from LRU side (tail) */
    struct cache_blk_t *ptr = set->way_tail;
    int steps = 0;

    /* Walk i steps toward MRU (head) */
    while (steps < i && ptr->way_prev != NULL)
    {
      ptr = ptr->way_prev;
      steps++;
    }
    ...
    /* --- Insertion --- */
    /* Insert before ptr (toward MRU) */
    blk->way_next = ptr;
    blk->way_prev = ptr->way_prev;

    if (ptr->way_prev)
      ptr->way_prev->way_next = blk;
    else
      set->way_head = blk; /* became new MRU if i large */

    ptr->way_prev = blk;

    /* Fix tail if we inserted before the old tail (i==0) */
    if (ptr == set->way_tail)
      set->way_tail = blk; /* became new LRU if i==0 */
  }
  else
                  ...
                  FILE *stream)       /* output stream */
{
  ...
  fprintf(stream,
          ...
          : cp->policy == FIFO   ? "FIFO"
          : cp->policy == MRU    ? "MRU"
          : cp->policy == LIP_4  ? "LIP_4"
          : cp->policy == LIP_8  ? "LIP_8"
                                 : (abort(), ""));
}
             ...
             md_addr_t *repl_addr) /* for address of replaced block */
{
  ...
  case MRU:
    // Evict the most recently used block (head)
    repl = cp->sets[set].way_head;
    break;

  case LIP_4:
  case LIP_8:
    // LIP evicts the same as LRU (tail)
    repl = cp->sets[set].way_tail;
    break;
  ...
  if (cp->policy == LIP_4)
  {
    fprintf(stderr, "using lip_4");
    update_way_list(&cp->sets[set], repl, Lip_4);
  }

  else if (cp->policy == LIP_8)

    update_way_list(&cp->sets[set], repl, Lip_8);
  else
...
cache_hit: /* slow hit handler */
  ...
  /* if LRU replacement or MRU and this is not the first element of list, reorder */
  if (blk->way_prev && (cp->policy == LRU || cp->policy == MRU || cp->policy == LIP_4 || cp->policy == LIP_8))
  {
